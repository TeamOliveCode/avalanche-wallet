 ...
 * WARNING!
 *
 * This MagicWallet is not fully tested.
 * Only several features are battle tested.
 */
...
class MagicWallet extends AbstractWallet implements AvaWalletCore, UnsafeWallet {
    ...
    constructor({
        ...
        this.ethKey = pkHex
        this.ethAddress = evmAddress
        this.publicAddress = publicAddress // X-fuji143rd4vf3z7uhy93n6ak25p56rn7nwkcthhueah
    ...
    async sendERC20(
        ...
        token.setGasLimit(gasLimit)

        const tx = token.createTransferTx(to, amount)

        const receipt = await tx.send({ from: this.ethAddress })
        const txHash = receipt.transactionHash
        return txHash
    }
